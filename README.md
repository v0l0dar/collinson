...
| 2026-09-08 | ~00:20-02:10 | Added a search icon and a clear button to the place field. Fixed a table that scrolled sideways at every width (the badges needed ~920px inside a 736px page). Redesigned the score table as a heatmap of colour-filled tiles, and moved each day's reasons out of the cells — they repeated seven times per row — into a hover tooltip plus a visually hidden copy for screen readers. Reviewed the round with three independent passes (correctness, architecture/DRY, accessibility/types) and fixed all 15 findings: a stuck tooltip, four text colours below WCAG 4.5:1 (one a regression from this round), a missing request order in the place lookup, a clear button that left the old forecast on screen, a 16x16px tap target, and a magic `min-w-190` that meant "7 days". Turned on `strict` TypeScript (zero errors) and added a `PlaceSearch` test file — the race test fails without its fix. |
